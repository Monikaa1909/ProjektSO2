...
void *writer_(void *arg) {
    ...
    while(1) {
        ...
        if(numberOfWritersInReadingRoom_ || numberOfReadersInReadingRoom_ > 0) {
            ...
            if((err = pthread_cond_wait(&writer, &mutex)) != 0) {
                fprintf(stderr, "cond_wait error = %d (%s)\n", err, strerror(err));
            }
            numberOfWritersInQueue_--;
            writersInQueue[nr] = 0;
        }
        ...
        if (numberOfReadersInQueue_ > 0) {
            if((err = pthread_cond_broadcast(&reader)) != 0) {
            ...
            if ((err = pthread_cond_broadcast(&writer)) != 0) {
                fprintf(stderr, "cond_broadcast error = %d (%s)\n", err, strerror(err));
            }
        }
        if ((err = pthread_mutex_unlock(&mutex)) != 0) {
...
void *reader_(void *arg) {
    ...
    while(1) {
        ...
        if(numberOfWritersInReadingRoom_ || numberOfWritersInQueue_ > 0) {
            ...
            if(pthread_cond_wait(&reader, &mutex) != 0) {
                fprintf(stderr, "cond_broadcast error = %d (%s)\n", err, strerror(err));
            }
            numberOfReadersInQueue_--;
            readersInQueue[nr] = 0;
        ...
        if (numberOfReadersInReadingRoom_ == 0) {
            if ((err = pthread_cond_signal(&reader)) != 0) {
